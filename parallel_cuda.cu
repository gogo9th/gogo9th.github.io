...
__global__ void kernel(char** normal_list, char** malware_list, int n_pos, int i, int j, int total_malware)
{
...
int main()
{
    ...
    for(n_pos = 6; n_pos <= 40; n_pos += 2) {
        ...
        while(  (read = getline(&line, &len, f_mal)) != -1 ) {
           ...
           malware_list[total_malware] = (char*)malloc(strlen(line_head) + 1);
        ...
        while(  (read = getline(&line, &len, f_app)) != -1 ) {
            ...
            normal_list[i] = (char*)malloc(strlen(line_head) + 1);
      ...
      cudaMalloc(&normal_list_d, sizeof(char*) * total_normal);
      cudaMalloc(&malware_list_d, sizeof(char*) * total_malware);

      for (int i = 0; i < total_normal; i++)
      {
         cudaMalloc(&(normal_list_d[i]), NORMAL_LEN);
         cudaMemcpy(&(normal_list_d[i]),  normal_list[i], strlen(normal_list[i]), cudaMemcpyHostToDevice);

      }


      for (int i = 0; i < total_malware; i++)
      {
         cudaMalloc(&(malware_list_d[i]), MALWARE_LEN);
         cudaMemcpy(&(malware_list_d[i]),  normal_list[i], strlen(normal_list[i]),cudaMemcpyHostToDevice);
        ...
        for (int i = 0; i < total_malware; i++){
            for (int j = 0; j < total_malware; j++){

               kernel<<<(total_malware + 255) / 256, 256>>>(normal_list_d, malware_list_d, n_pos, i, j, total_malware);
        ...
        printf("processing time for in sec: %4.2f\n", time1);
         ...
         cudaFree(normal_list_d);
         cudaFree(malware_list_d);
